  ...
  persist(
    (set, get) => ({
      ...
      setLeadStage: (leadId, stage) => {
        ...
        // Async Supabase Cloud Update
        supabase
          .from("leads")
          .update({ stage })
          .eq("id", leadId)
          .then(({ error }) => {
            if (error) console.error("Supabase stage update error:", error);
          });
      },
          ...
          autoAssignFn(
            {
              ...
              source: input.source ?? "Direct Ingestion",
